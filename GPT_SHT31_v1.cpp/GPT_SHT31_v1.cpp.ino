...
static uint8_t crc8_sht(const uint8_t *data, int len) {		// CRC-8 for SHT3x
...
void raiseFault(FaultCode c, Severity s, bool latched);       // cause une erreur si place plus bas dans le code... 
...
public:
  ...
  void acknowledgeLatched() {
    ...
    for (auto &f : faults) {
      Serial.print("  active=");
      Serial.print(f.active ? "ON\n" : "OFF\n");
//  lcd.setCursor(0, 0);
//  lcd.print("Fault.active: ");
//  lcd.print(f.active, 1);
//  lcd.setCursor(0, 1);
//  lcd.print("Fault.latched: ");
//  lcd.print(f.latched, 1);
...
static constexpr uint16_t LOG_CAPACITY    = 4096;     //256; // records; adjust to your FRAM size
...
void loop() {
  ...
  if (d.mustSafeStop) {
    ...
    // Simple test behavior: toggle SSR ON if temp < (OVERTEMP_C - CONTROL_ON_DELTA)
    if (sht.hasFresh && isfinite(sht.tempC) && sht.tempC < (OVERTEMP_C - CONTROL_ON_DELTA)) {
      ssrCommandedOn = true;
      ssrWrite(true);
    } else {
      ssrCommandedOn = false;
  ...
  if (now - lastPrint > 1000) {
    ...
    Serial.print("  PIN_ACK=");
    Serial.print(digitalRead(PIN_ACK) ? "ON" : "OFF");
...
//    lcd.setCursor(0, 0);
//    lcd.print("Fault.active: ");
//    lcd.print(faults.active, 1);
//    lcd.setCursor(0, 1);
//    lcd.print("Fault.latched: ");
//    lcd.print(faults.latched, 1);
